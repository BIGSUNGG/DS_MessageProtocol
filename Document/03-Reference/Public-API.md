...
생성기가 partial에 구현을 붙이거나, 수동으로 동일 형태를 노출한 뒤 등록한다. 수동 구현 시 헤더는 와이어 순서(헤더 바이트 → ID 3바이트)로 직접 기록해야 한다. 판독 시에도 헤더를 검증해 타입 MessageId 와 비교할 것을 권장한다 — 생성 구현은 2.3.2 부터 이 검증으로 다른 타입 바이트의 조용한 재해석을 거부한다(KI-5).
